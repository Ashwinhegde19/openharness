...
export async function assertClaudeContextLimitRetry(context: TestContext): Promise<void> {
  ...
  try {
    ...
    assert(
      stderr.includes("[openharness proxy] context-fit retry") ||
        ...
        (stderr.includes("openharness: trimmed") && stderr.includes("(retry path")) ||
        (stderr.includes("openharness: DROPPED A LARGE PORTION") && stderr.includes("(retry path")),
...
export async function assertCodexContextLimitRetry(context: TestContext): Promise<void> {
  ...
  try {
    ...
    assert(!looksLikeContextError(text), "context-length error leaked to the client");
    assert(
      daemon.stderr().includes("[openharness proxy] context-fit retry"),
