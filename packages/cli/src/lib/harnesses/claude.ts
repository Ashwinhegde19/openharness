...
async function resolveClaudeProvider(ctx: HarnessContext): Promise<{
  provider: ProviderConfig;
  endpoint: ProviderEndpointConfig;
  apiKey: string;
  models: ProviderModel[];
}> {
  const providerId = (ctx.provider ?? CLAUDE_DEFAULT_PROVIDER_ID).trim().toLowerCase();
  if (!isBuiltinProviderId(providerId) && providerId !== "togetherai") {
    throw new Error(
      `Unknown provider "${ctx.provider}". Supported for Claude: together, openrouter, ollama.`,
    );
  }
  ...
  if (providerId === OPENROUTER_PROVIDER_ID) {
    ...
    if (!apiKey) {
      throw new Error(
        "Claude + OpenRouter needs OPENROUTER_API_KEY (or --api-key / configure). " +
...
function resolveClaudeLaunchModel(
  providerId: string,
  models: ProviderModel[],
  ...
  if (!id) {
    throw new Error(
      `No model available for Claude on provider "${providerId}". Pass --main <model-id>.`,
    );
  }
  ...
  return {
    alias: claudeAliasForUpstreamId(id),
    targetModelId: id,
...
export default defineHarness({
  ...
  async run(ctx) {
    ...
    const resolved = await resolveClaudeProvider(effective);
    const providerId = resolved.provider.id;
    const model = resolveClaudeLaunchModel(providerId, resolved.models, effective.main);
    ...
    const result = await runClaudeTogether({
