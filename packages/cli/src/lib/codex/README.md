...
`ohcodex` launches the native Codex CLI through a local TogetherLink proxy. The adapter keeps two kinds of configuration separate:
...
The only generic config write allowed by `ohcodex` is the first-run safety seed: if `~/.codex/config.toml` is missing or empty, create it with Codex's "Auto + approve for me" posture:
